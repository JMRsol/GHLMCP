...
Returns: list of matching contacts with their IDs and metadata.`,
        inputSchema: zod_1.z.object({
            query: zod_1.z.string().min(1).describe("Name, email, or phone to search for"),
        ...
        try {
            const client = (0, ghl_js_1.getClient)();
            ...
            return { content: [{ type: "text", text: (0, ghl_js_1.truncate)(JSON.stringify({ total: data.total, page, contacts: output }, null, 2)) }] };
        }
        catch (e) {
...
Args:
  - contactId: the GHL contact ID string`,
        inputSchema: zod_1.z.object({
            contactId: zod_1.z.string().min(1).describe("GHL contact ID"),
        }),
        annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false },
    }, async ({ contactId }) => {
        try {
            const { data } = await (0, ghl_js_1.getClient)().get(`/contacts/${contactId}`);
            return { content: [{ type: "text", text: (0, ghl_js_1.truncate)(JSON.stringify(data.contact ?? data, null, 2)) }] };
        }
        catch (e) {
            return { content: [{ type: "text", text: (0, ghl_js_1.formatError)(e) }], isError: true };
        }
...
Args:
  ...
  - customFields: key-value map of custom field updates`,
        ...
        try {
            ...
            const { data } = await (0, ghl_js_1.getClient)().put(`/contacts/${contactId}`, payload);
            return { content: [{ type: "text", text: JSON.stringify(data.contact ?? data, null, 2) }] };
        }
        catch (e) {
            return { content: [{ type: "text", text: (0, ghl_js_1.formatError)(e) }], isError: true };
...
Args:
  ...
  - tags: array of tag strings to add`,
        ...
        try {
            const { data } = await (0, ghl_js_1.getClient)().post(`/contacts/${contactId}/tags`, { tags });
...
Args:
  ...
  - tags: array of tag strings to remove`,
        ...
        try {
            const { data } = await (0, ghl_js_1.getClient)().delete(`/contacts/${contactId}/tags`, { data: { tags } });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (e) {
            return { content: [{ type: "text", text: (0, ghl_js_1.formatError)(e) }], isError: true };
...
Args:
  ...
  - body: the note text content`,
        inputSchema: zod_1.z.object({
            contactId: zod_1.z.string().min(1),
            body: zod_1.z.string().min(1).describe("Note content"),
        }),
        annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false },
    }, async ({ contactId, body }) => {
        try {
            const userId = process.env.GHL_USER_ID ?? "";
            const { data } = await (0, ghl_js_1.getClient)().post(`/contacts/${contactId}/notes`, { body, userId });
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        catch (e) {
            return { content: [{ type: "text", text: (0, ghl_js_1.formatError)(e) }], isError: true };
